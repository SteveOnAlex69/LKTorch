...
PYBIND11_MODULE(lktorch, m) {
    ...
    py::class_<Tensor>(m, "Tensor")
        ...
        .def("get_tensor_size", &Tensor::get_tensor_size,
            "Returns the total number of elements flattened inside the tensor.")
        .def("get_tensor_dimension", &Tensor::get_tensor_dimension,
        ...
        // --- CORE AUTOGRAD/MEMORY OPERATIONS ---
        ...
        .def("Permute Dimension", &Tensor::PermuteDimension, py::arg("target_dimension"),
        ...
        // --- NATIVE PYTHON MATHEMATICAL OPERATORS ---
        .def(py::self + py::self)
        .def(py::self - py::self)
        .def(py::self * py::self)
    ...
    m.def("Permute", &Permute, py::arg("tensor"), py::arg("target_dimension"), py::arg("target_permutation"),
        "Permuting the element in the tensor however you like");
    m.def("Permute Dimension", &PermuteDimension, py::arg("tensor"), py::arg("target_dimension"),
        "Swapping the dimensions around.");
    ...
    py::class_<TensorFunction>(m, "TensorFunction", "Base class for differentiable activations.")
        // Default constructor
        .def(py::init<>())
        // FIX: The constructor that lets you pass the functions in as variables!
        // This allows: custom_func = lk.TensorFunction(my_f, my_df)
        ...
        // Resolving the C++ member function overload for .forward()
        ...
        // Resolving the C++ member function overload for operator() -> __call__
    ...
    // ========================================================================
    // By binding these as attributes, Python treats them exactly like PyTorch's F.relu
    ...
    py::class_<Module, std::shared_ptr<Module>>(m, "Module", "Abstract base class for all neural network modules.")
        // Polling parameters and state dictionaries
        .def("get_parameters", &Module::get_parameters, "Returns a list of trainable parameter Tensors.")
        .def("get_state_dict", &Module::get_state_dict, "Extracts raw float data arrays representing module weights.")
        .def("load_state_dict", &Module::load_state_dict, py::arg("state_dict"), "Restores weights from raw float data matrix layouts.")

        // Target forward mechanics
    ...
    // Parameterized Operational Layers
    ...
    py::class_<Sequential, Module, std::shared_ptr<Sequential>>(m, "Sequential", "Chains modules into linear structural pipelines.")
        ...
        // Binds standard indexing accessor methods to the [] operator implementation
    ...
    py::class_<GradientDescent, std::shared_ptr<GradientDescent>>(m, "GradientDescent", "Standard Stochastic Gradient Descent.")
        ...
        // Handling the overloaded add_parameter functions
    ...
    py::class_<AdaGrad, GradientDescent, std::shared_ptr<AdaGrad>>(m, "AdaGrad", "Adaptive Gradient Algorithm.")
        .def(py::init<float>(), py::arg("learning_rate") = 0.0f)
        // AdaGrad explicitly overrides add_parameter, so we must bind its specific pointers
